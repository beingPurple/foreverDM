...
PROMPT_FILE = f'{current_path}/prompt/default/{CHAT_LANG}-foreverDM.py'
...
def on_message(message,srv,FREE_GEN_LEN=256):
    ...
    #         print("Path error.")
        ...
        if msg[:3].lower() == '+i ':
            ...
            new = f'''
Below is a request from the related to the game so far. Write a response that appropriately completes the request.
###Instruction:
{msg}
###Response:
'''
        ...
        #     output = pipeline.decode(model_tokens[begin:]).strip()
            ...
            ins = '\nThe player has a question about the game so far. Provide a response that answers the question.'
            new = f"{ins} {user}{interface}{real_msg}\n\n{bot}{interface}"
            print(new)
            out = run_rnn(pipeline.encode(new))
            save_all_stat(srv, 'gen_0', out)
            begin = len(model_tokens)
